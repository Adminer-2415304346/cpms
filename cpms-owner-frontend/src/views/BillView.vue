<template>
  <div class="bill-page">
    ...
    <a-card v-for="group in billGroups" :key="group.roomId" :title="`${group.buildingName} ${group.unitName || ''} ${group.roomNumber}`" style="margin-bottom:16px" size="small">
      <a-table :dataSource="group.bills" :pagination="false" rowKey="billId" size="small">
        ...
        <a-table-column title="金额" dataIndex="amount" width="140">
          <template #default="{ record }">
            <span class="bill-amount">¥{{ record.amount?.toFixed(2) }}</span>
          </template>
        </a-table-column>
    ...
    <a-modal v-model:open="payModal.open" title="确认缴费" @ok="doPay" :confirmLoading="payModal.loading">
      <a-form layout="vertical">
        <a-form-item label="账单信息">
          <p>费用类型：{{ payModal.bill?.feeType }}</p>
          <p>
            金额：<strong class="bill-amount">¥{{ payModal.bill?.amount?.toFixed(2) }}</strong>
          </p>
        </a-form-item>
        <a-form-item label="支付方式" required>
          <a-select v-model:value="payModal.payMethod" placeholder="选择支付方式" style="width:100%">
...
async function doPay() {
  ...
  try {
    await http.post(`/bills/${payModal.bill.billId}/pay`, {
      amount: payModal.bill.amount,
      payMethod: payModal.payMethod
    })
